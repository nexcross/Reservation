<?php
    ...
    if(!($queryResult = mysqli_query($connection, "SELECT distinct(room.room_no), room_type, room_charge FROM room LEFT JOIN reservation on room.room_no = reservation.room_no WHERE reservation.room_no NOT IN (SELECT room_no FROM reservation WHERE odate = '$cdate')  or reservation.room_no IS NULL"))){
